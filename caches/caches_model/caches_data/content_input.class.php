...
class content_input {
	...
	$pictures = $_POST[$field.'_url'];//å–å¾—è§†é¢‘åˆ—è¡¨
...
foreach($arrurl as $ka=>$pa){
		  ...
		  ++$k;
          $arrv = explode('$',$pa);
		  $temp0['url'] = $arrv[1]?$arrv[1]:$arrv[0];//æ‡’å®¶ä¼™æ²¡æœ‰|
		  $temp0['alt'] = $arrv[1]?$arrv[0]:'ç¬¬'.$k.'é›†';
		  $temp['p'] = $playerid;
		  $array0[$ka] = $temp0;
		  }
		}
		...
		if(!empty($pictures)){
		$pictures_alt = isset($_POST[$field.'_alt']) ? $_POST[$field.'_alt'] : array();		//å–å¾—è§†é¢‘è¯´æ˜Ž
		if(!empty($pictures)) {
			foreach($pictures as $key=>$pic) {
				$temp['url'] = $pic;
				$temp['alt'] = $pictures_alt[$key]?$pictures_alt[$key]:'ç¬¬'.++$key.'é›†';
				$temp['p'] = $playerid;
				$array[$key] = $temp;
			}
		}
