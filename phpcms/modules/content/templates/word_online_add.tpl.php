...
<form name="myform" id="myform" action="?m=content&c=word_online&a=add" method="post">
  <div class="pad_10">
    <table width="100%" class="table_form ">
      <tr>
        <th><?php echo L('title')?>：</th>
        <td><input type="text" name="info[title]" id="title" class="input-text" value="" size="50" onblur="$.post('api.php?op=get_keywords&number=5&sid='+Math.random()*5, {data:$('#title').val()}, function(data){if(data && $('#keywords').val()=='') $('#keywords').val(data); })"></td>
      </tr>
      <tr>
        <th><?php echo L('keywords')?>：</th>
        <td><input type="text" name="info[keywords]" id="keywords" class="input-text" value="" size="50"></td>
      </tr>
      <tr>
        <th><?php // echo L('keywords')?>相关人物：</th>
        <td><input type="text" name="info[relative_people]" id="relative_people" class="input-text" value="" size="50"></td>
      </tr>
      <tr>
        <th><?php echo "直播缩略图片"; // L('word_online_image'); ?>：</th>
        <td><?php echo form::images('info[thumb]', 'thumb', '', 'content');?></td>
      </tr>
      <tr>
  ...
  <div class="fixed-bottom">
    <div class="fixed-but text-c">
      ...
      <div class="button"><input value="<?php echo L('save_and_online');?>" type="submit" name="dosubmit_online" class="cu" style="width:130px;"></div>
      <div class="button"><input value="<?php echo L('save_continue');?>" type="submit" name="dosubmit_continue" class="cu" style="width:130px;"></div>
    </div>
  </div>
</form>
